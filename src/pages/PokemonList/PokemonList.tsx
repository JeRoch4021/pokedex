    ...
    <div className="flex flex-col h-full bg-red-500">
      ...
      <div className="px-4 py-2 mb-2">
        <div className="relative flex items-centers">
          <Search className="absolute right-3 bottom-3 text-gray-400 pointer-events-none" />
          <Input className="bg-white rounded-full h-12" placeholder="Find..." />
        </div>
      </div>
      <div className="grow bg-white m-2 p-2 rounded-sm overflow-y-auto">
        <div className="grid grid-cols-3 gap-4 p-4 ">
            {pokemons.map(({ id, name }, index) => (
            <PokemonThumbnail key={index} id={id} name={name} />
            ))}
        </div>
      </div>
    </div>
